...
def audit_post(post, html_content):
    ...
    url = post.get("canonical_url") or f"https://borjamoskv.substack.com/p/{post.get('slug')}"
    ...
    c5_real_count = sum(len(re.findall(pat, text_to_search)) for pat in C5_REAL_PATTERNS)
...
def main():
    ...
    to_fetch = []
    for p in posts_meta:
        url = p.get("canonical_url") or f"https://borjamoskv.substack.com/p/{p.get('slug')}"
        if url not in html_cache or not html_cache[url]:
            to_fetch.append((p.get("title"), url))
            
    if to_fetch:
        print(f"[*] Need to fetch {len(to_fetch)} new/missing posts HTML from Substack...")
        cache_updated = False
        
        # Parallel fetcher
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(fetch_post_html, url): (title, url) for title, url in to_fetch}
            completed_count = 0
                ...
                try:
                    ...
                    if html:
                        ...
                        if completed_count % 10 == 0 or completed_count == len(to_fetch):
                            print(f"    Fetched {completed_count}/{len(to_fetch)} posts...")
        ...
        print("[✓] All posts HTML available in local cache. Zero network requests required for body text.")
    ...
    bottom_10 = audited_posts[-15:][::-1] # Worst sorted by worst (lowest exergy / highest smoke)
    
    # Exergy classifications
    c5_real_posts = [p for p in audited_posts if p["classification"] == "C5-REAL / Arquitectura"]
    tech_posts = [p for p in audited_posts if p["classification"] == "Técnico / Práctico"]
    comm_posts = [p for p in audited_posts if p["classification"] == "Comercial / Transaccional"]
    hype_posts = [p for p in audited_posts if p["classification"] == "Hype-driven / Divulgativo"]
