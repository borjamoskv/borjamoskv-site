...
def calculate_exergy_metrics(raw_count, processed_count, time_per_item_mins=2.0):
    ...
    exergy_efficiency = 100.0 if raw_count == 0 else (processed_count / raw_count) * 100.0
    
    return {
        "manual_hours_saved_month": round(manual_hours_month, 2),
        "exergy_efficiency": round(exergy_efficiency, 2),
        "entropy_reduction_ratio": round(1.0 - (auto_hours_month / (manual_hours_month or 1.0)), 4)
    }
...
def verify_pipeline(folder_path):
    ...
    if not path.exists():
        print(f"[ERROR] Directorio '{folder_path}' no existe. Creando entorno simulado C4-SIM para validar...")
    ...
    for file_path in files:
        try:
            ...
            if missing:
                print(f"[WARNING] Archivo {file_path.name} corrupto. Faltan claves: {missing}")
                corrupted_count += 1
            else:
                valid_count += 1
    ...
    print(f"Horas mensuales disipadas salvadas (para 197 items/mes): {metrics['manual_hours_saved_month']} hrs")
    print(f"Eficiencia Exérgica de Captura: {metrics['exergy_efficiency']}%")
    print(f"Reducción de Entropía Operativa: {metrics['entropy_reduction_ratio'] * 100:.2f}%")
    print("=========================================================")
