...
def load_seeds():
    if os.path.exists(CACHE_IDS_PATH):
        try:
            with open(CACHE_IDS_PATH, encoding="utf-8") as f:
                return json.load(f)
...
def audit_text(title, subtitle, description, lang):
    ...
    if lang == "en":
        cb_pats, tech_pats, comm_pats, c5_pats = (
            EN_CLICKBAIT,
            EN_TECH,
            EN_SPONSOR,
            EN_C5_REAL,
...
def audit_publication(subdomain, base_url):
    ...
    for p in posts:
        ...
        langs.append(lang)
        exergy, smoke, cb, tech, comm, c5 = audit_text(title, subtitle, desc, lang)
...
def main():
    ...
    for sub, info in graph.items():
        ...
        for rec in info.get("recommendations", []):
            ...
            tid = rec.get("id")
            turl = rec.get("custom_domain") or (
                f"https://{tsub}.substack.com" if tsub else None
            )

            # If not found, try raw structure
            if not tsub or not tid:
                target = (
                    rec.get("recommendedPublication") or rec.get("target_pub") or {}
                )
                tsub = target.get("subdomain")
                tid = target.get("id")
                turl = target.get("custom_domain") or (
                    f"https://{tsub}.substack.com" if tsub else None
                )
    ...
    print(
        f"[*] Pre-populated {len(discovered)} unique subdomains from cache graph and seeds."
    )
    ...
    print(
        f"[*] Starting crawl to reach target of {MAX_DISCOVERED} discovered subdomains..."
    )
    ...
    while queue and len(discovered) < MAX_DISCOVERED:
        ...
        if recs:
            ...
            graph[sub] = {"subdomain": sub, "recommendations": recs}
            new_recs_count += 1
            ...
            for rec in recs:
                ...
                if target_sub and target_id and target_sub not in discovered:
                    target_url = clean_url(
                        target.get("custom_domain")
                        or f"https://{target_sub}.substack.com"
                    )
    ...
    crawl_end = time.time()
    print(
        f"[✓] Crawl complete. Discovered {len(discovered)} subdomains (New recommendation nodes fetched: {new_recs_count}) in {crawl_end - crawl_start:.2f} seconds."
    )
    ...
    print(
        f"[*] Launching parallel exergy audit on {len(audit_list)} discovered subdomains (Concurrency: {CONCURRENCY}, Target: {MAX_AUDITED_TARGET})..."
    )
    ...
    print(
        f"[*] Re-used {len(audited_results)} cached audits. Need to audit {len(to_audit_list)} subdomains on network."
    )
    ...
    if to_audit_list:
        with ThreadPoolExecutor(max_workers=CONCURRENCY) as executor:
            futures = {
                executor.submit(audit_publication, sub, discovered[sub]["url"]): sub
                for sub in to_audit_list
            }
            completed = 0
            for completed, future in enumerate(as_completed(futures), 1):
                ...
                if completed % 100 == 0 or completed == len(to_audit_list):
                    print(
                        f"    Audited {completed}/{len(to_audit_list)} publications (Successful: {len(audited_results)})..."
                    )

    scan_end = time.time()
    print(
        f"[✓] Audit complete. Processed {len(audited_results)} successful publications (including cache) in {scan_end - scan_start:.2f} seconds."
    )

    # Sort results by exergy descending, sub-sorting by tech and c5_real descending
    audited_results.sort(
        key=lambda x: (x["exergy"], x["tech"], x["c5_real"]), reverse=True
    )

    # Retain all successful results for complete analysis
    final_results = audited_results
    print(
        f"[*] Retained all {len(final_results)} audited publications for the final report."
    )
    ...
    c5_pubs = [
        x for x in final_results if x["classification"] == "C5-REAL / Arquitectura"
    ]
    tech_pubs = [
        x for x in final_results if x["classification"] == "Técnico / Práctico"
    ]
    comm_pubs = [
        x for x in final_results if x["classification"] == "Comercial / Transaccional"
    ]
    hype_pubs = [
        x for x in final_results if x["classification"] == "Hype-driven / Divulgativo"
    ]
