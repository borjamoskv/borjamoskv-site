...
const handler: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (!kv) {
      try {
        kv = (locals as any)?.runtime?.env?.CORTEX_ENTROPY;
      } catch (e) {
        // Astro v6 throws on locals.runtime
      }
    }
    ...
    const ledgerPath = path.resolve(process.cwd(), 'substack_archive/exergy_ledger.jsonl');
