...
def evaluate_node(node: dict) -> dict:
    ...
    centralization_ratio = node.get("centralization_ratio", 0.5)  # [0.0 - 1.0] (1.0 = full dependency on central APIs)
    interception_risk = node.get("interception_risk", 0.5)        # [0.0 - 1.0] (lack of encryption or scrubbing)
    local_persistence = node.get("local_persistence", 0.5)        # [0.0 - 1.0] (SQLite, Git checkpoints, ROM partition)
    mesh_discovery = node.get("mesh_discovery", 0.5)              # [0.0 - 1.0] (Dynamic peer discovery / DHT)
    ...
    exergy = (persona_libro_score * 0.5 + granger_score * 0.5) * (1.0 - (centralization_ratio * 0.6 + interception_risk * 0.4) * 0.8)
...
def audit_network(nodes: list) -> dict:
    ...
    verdict = "VERIFIED - Resilient Decentralized Knowledge Mesh" if avg_exergy >= 60 else "REJECTED - Firehouse Centralization Detected"
