...
def evaluate_goal(goal: dict) -> dict:
    ...
    exergy = (intrinsic_drive * 0.6 + proof_of_work * 0.4) * (
        1.0 - (mimetic_factor * 0.5 + anergy_cost * 0.5) * 0.7
    )
...
def audit_list(goals: list) -> dict:
    ...
    verdict = (
        "VERIFIED - Sovereign Agenda"
        if avg_exergy >= 65
        else "REJECTED - High Mimetic Contamination"
    )
