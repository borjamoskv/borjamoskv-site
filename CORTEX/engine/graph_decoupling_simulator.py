...
def simulate_platform_decay(years=15, initial_followers=1000):
    ...
    print(f"Iniciando simulación: {years} años. Base inicial: {initial_followers} Nodos.\n")
    ...
    for year in range(1, years + 1):
        ...
        status = "Social-Dominant" if social_graph_weight > content_graph_weight else "Algorithmic-Dominant"
        
        if year % 3 == 0 or year == years:
            print(f"[Año {year}] Estado: {status}")
            print(f"  -> Peso Social: {social_graph_weight:.2f} | Peso Algorítmico: {content_graph_weight:.2f}")
            print(f"  -> Alcance Orgánico Garantizado: {max(organic_reach*100, 1.0):.2f}% ({reached_fans} Nodos contactados)")
            print("-" * 50)
