...
class TacuinumEngine:
    ...
    def ingest(self, input_data: MetabolicInput) -> bool:
        ...
        if input_data.anergy_cost > 50:
            self.anergy_traps += 1
            self.logs.append(f"[FALLA C4] RECHAZO: '{input_data.source}' genera fuga de Exergy ({input_data.anergy_cost}). Voltaje: {self.voltage}.")
            return False
            
        self.logs.append(f"[C5-REAL] ACEPTADO: '{input_data.source}' aporta Soberanía ({input_data.exergy_yield}). Voltaje: {self.voltage}.")
        return True
...
def run_audit():
    ...
    inputs = [
        ...
        MetabolicInput("Hormesis_Estival_Sudor", "enfriamiento_activo", anergy_cost=10, exergy_yield=80),
        MetabolicInput("Superfood_Importado", "variable", anergy_cost=40, exergy_yield=20),
        MetabolicInput("Proximidad_Aceite_Crudo", "calentamiento_metabolico", anergy_cost=5, exergy_yield=95),
        MetabolicInput("Friccion_Social_Noticias", "calentamiento_nervioso", anergy_cost=90, exergy_yield=0),
        MetabolicInput("Ecologia_Del_Silencio", "enfriamiento_nervioso", anergy_cost=0, exergy_yield=100)
    ]
