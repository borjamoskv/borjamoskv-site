...
def audit_quixote_delusion(runtime_state: dict) -> dict:
    ...
    unverified_assumptions = runtime_state.get(
    ...
    useless_metric_weight = runtime_state.get(
        "useless_metric_weight", 0.0
    )  # [0.0 - 1.0]
    real_work_ratio = runtime_state.get("real_work_ratio", 1.0)  # [0.0 - 1.0]

    dulcinea_index = (
        useless_metric_weight * 0.7 + (1.0 - real_work_ratio) * 0.3
    ) * 100.0

    # Calculate System Exergy (Ideal balance is low delusion, high grounding, low escalation, low phantom)
    exergy_score = (
        (100.0 - quixote_index) * 0.3
        + sancho_grounding * 0.4
        + (100.0 - windmill_escalation) * 0.15
        + (100.0 - dulcinea_index) * 0.15
    )
